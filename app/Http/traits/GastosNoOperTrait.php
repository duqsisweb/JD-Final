    ...
    public function tablaGastosNoOperacionales($fechaIni, $fechaFin)
    {
        ...
        foreach ($infoNoOpe as $data) {
            if ($c == 3 || $c == 7 || $c == 11 || $c == 15) {
                ...
                $finan = $data->FINANCIEROS;
                $porceFinan = round($finan * 100 / $TOTALV, 2) . '%';
                $retActiv = $data->RETIRO_ACTIVOS;
                $porceActiv = round($retActiv * 100 / $TOTALV, 2) . '%';
                $gravFinan = $data->GRAVA_MOV_FINANCIERO;
                ...
                array_push($fomDates, [
                    intval(round($finan)), $porceFinan, intval(round($retActiv)), $porceActiv, intval(round($gravFinan)), $porceGravFin, intval(round($otros)), $porceOtros,
                    intval($totlNoOp), $porceTotlNoOp, intval(round($utilAntImp)), $porceUtilAntImp, intval($ebitda), $porceEbtida
                ]);
                $c++;
        ...
        for ($i = 0; $i < count($sumados[0]); $i++) {
            ...
            array_push($acumulados, intval($sumNo));
        ...
        for ($i = 0; $i < count($sumados[0]); $i++) {
            ...
            array_push($promediosNo, intval($promNo));

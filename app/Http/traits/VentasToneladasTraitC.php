...
trait VentasToneladasTraitC
{

    public function TablaVentasToneladasC()
    {
        ...
        foreach ($infoTons as $info) {
            if ($c == 3 || $c == 7 || $c == 11 || $c == 15) {
                ...
                $aceitesOP = $info->TON_ACEITES;
                $aceites = $info->TON_ACEITES;
                $margarinasOP = $info->TON_MARGARINAS;
                $margarinas = $info->TON_MARGARINAS;
                $soliCremOP = $info->TON_SOLIDOS_CREMOSOS;
        ...
        foreach($acumulados as $ac){
            array_push($promedios, round($ac / count($infoTons)));
        }
